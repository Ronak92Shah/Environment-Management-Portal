...
//Open the connection
	...
	$dbSelect = @mysqli_select_db($dbConnect, "environment_management_portal");
		...
		if(mysqli_num_rows($result)==1)
		{
				...
				$query = "SELECT releasePhaseName,releaseStartDate,releaseEndDate FROM releasepackage where releasePkgName = '$name' ";
				...
				while ($result_query = mysqli_fetch_array($query_select))
				{
					...
					$start_date = date_create($result_query["releaseStartDate"]);
					$start_date = date_format($start_date,"d-m-Y");
					array_push($array_data,$start_date);
					
					$end_date = date_create($result_query["releaseEndDate"]);
